...
def intentar_login(email_input: str, password_input: str) -> bool:
    ...
    """
    # Intentamos la autenticación con la API de Supabase
    res = database.supabase.auth.sign_in_with_password({
        "email": email_input.strip(), 
        "password": password_input
    })
    
    # Si Supabase no lanza excepción, el login es correcto: inyectamos estados
    st.session_state.autenticado = True
    st.session_state.taller_id = res.user.id
    st.session_state.email = res.user.email
    st.session_state.supabase_session = res.session  # guardamos sesión para update_user

    # Persistimos la sesión guardando strings en el navegador
    _cookies.set("session_autenticado", "True")
    _cookies.set("session_taller_id", str(res.user.id))
