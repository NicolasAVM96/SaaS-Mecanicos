...
- **2026-06-16**: Multi-tenancy base — tablas `talleres` y `perfiles` + aislamiento RLS. Se crearon las tablas `talleres` (raíz multi-tenant) y `perfiles` (extiende auth.users con `taller_id` y `rol`). Se agregó `taller_id` como columna NOT NULL a `clientes`, `vehiculos` y `servicios`. El índice único de patente pasó de global a compuesto `(patente, taller_id)` para que dos talleres distintos puedan tener el mismo vehículo. Se creó la función helper `public.auth_taller_id()` (SECURITY DEFINER) y se reemplazaron las políticas RLS permisivas (`qual: true`) por políticas que filtran por `taller_id = auth_taller_id()`. En código: `auth_controller.py` ahora consulta `perfiles` tras el login para cargar `taller_id` (UUID del taller), `rol` y `nombre` en session_state; `database.py` inyecta `taller_id` en todos los inserts y filtra por él en `buscar_vehiculo_por_patente()`; `record_controller.py` y `record_form.py` propagan `taller_id` desde session_state. El sidebar muestra el rol del usuario. Objetivo: aislar completamente los datos entre talleres.
